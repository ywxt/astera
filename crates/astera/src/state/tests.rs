...
#[test]
fn fractional_scale_can_be_recreated_and_duplicate_is_rejected_without_server_panic() {
    let mut display = Display::<Astera>::new().unwrap();
    let mut state = Astera::new(&display.handle(), Config::default());
    let (server_socket, client_socket) = UnixStream::pair().unwrap();
    display
    ...
    let client = thread::spawn(move || {
        ...
        let manager = globals
            ...
            .unwrap();
        let surface = compositor.create_surface(&queue, ());
        let first = manager.get_fractional_scale(&surface, &queue, ());
        first.destroy();
        let _replacement = manager.get_fractional_scale(&surface, &queue, ());
        connection.flush().unwrap();
...
#[test]
fn fifo_can_be_recreated_and_duplicate_is_rejected_without_server_panic() {
    let mut display = Display::<Astera>::new().unwrap();
    let mut state = Astera::new(&display.handle(), Config::default());
    let (server_socket, client_socket) = UnixStream::pair().unwrap();
    display
    ...
    let client = thread::spawn(move || {
        ...
        let manager = globals
            ...
            .unwrap();
        let surface = compositor.create_surface(&queue, ());
        let first = manager.get_fifo(&surface, &queue, ());
        first.destroy();
        let _replacement = manager.get_fifo(&surface, &queue, ());
        connection.flush().unwrap();
        recreated_tx.send(()).unwrap();
        recreate_ok_tx
            .send(events.roundtrip(&mut TestClient).is_ok())
        ...
        let _duplicate = manager.get_fifo(&surface, &queue, ());
        connection.flush().unwrap();
        duplicate_tx.send(()).unwrap();
        error_tx
            .send(events.roundtrip(&mut TestClient).is_err())
...
#[test]
fn commit_timer_can_be_recreated_and_duplicate_is_rejected_without_server_panic() {
    let mut display = Display::<Astera>::new().unwrap();
    let mut state = Astera::new(&display.handle(), Config::default());
    let (server_socket, client_socket) = UnixStream::pair().unwrap();
    display
    ...
    let client = thread::spawn(move || {
        ...
        let timing = globals
            ...
            .unwrap();
        let surface = compositor.create_surface(&queue, ());
        let first = timing.get_timer(&surface, &queue, ());
        first.destroy();
        let _replacement = timing.get_timer(&surface, &queue, ());
        connection.flush().unwrap();
        recreated_tx.send(()).unwrap();
        recreate_ok_tx
            .send(events.roundtrip(&mut TestClient).is_ok())
        ...
        let _duplicate = timing.get_timer(&surface, &queue, ());
        connection.flush().unwrap();
        duplicate_tx.send(()).unwrap();
        error_tx
            .send(events.roundtrip(&mut TestClient).is_err())
...
#[test]
fn duplicate_xdg_dialog_is_rejected_without_server_panic() {
    let mut display = Display::<Astera>::new().unwrap();
    let mut state = Astera::new(&display.handle(), Config::default());
    let (server_socket, client_socket) = UnixStream::pair().unwrap();
    display
    ...
    let client = thread::spawn(move || {
        ...
        let _first = dialogs.get_xdg_dialog(&toplevel, &queue, ());
        let _duplicate = dialogs.get_xdg_dialog(&toplevel, &queue, ());
        connection.flush().unwrap();
        sent_tx.send(()).unwrap();
        error_tx
            .send(events.roundtrip(&mut TestClient).is_err())
